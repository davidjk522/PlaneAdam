...
class RandomFlip(Base):
    ...
    def tf(self,img,k=0): # img shape is (1, 240, 240, 155, 4)(1,128,128,16)
        if self.x_buffer:
...
class RandomRot90(Base):
    ...
    def sample(self, *shape):
        shape = list(shape)
        i, j = self.axes
        self.rotate_num = np.random.randint(0,4)
    ...
    def tf(self, img, k=0):
        ...
        img = np.rot90(img, self.rotate_num,axes=self.axes)
        return img
