...
class FMIR(nn.Module):
    ...
    def __init__(self,
        ...
        cv_ks = '1',
        is_int = '1',
        # in_channels = fixed/moving feature channels (e.g. PCA-reduced DINO dim) + 1 image
        # channel. 769 was this file's original default (768-dim features + 1 image channel);
        # kept as the default for backward compatibility, but callers with a different feature
        # dim (e.g. PCA-reduced DINOv3 features) should pass the right value explicitly.
        in_channels = '769',
        #up_scale = '(2,2,1)'
    ):
        ...
        print("img_size: {}, start_channel: {}, lk_size: {}, cv_ks: {}, is_int: {}, in_channels: {}".format(self.img_size, self.start_channel, self.lk_size, self.cv_ks, self.is_int, self.in_channels))

        N_s = self.start_channel
        self.simple_encoder = nn.Sequential(
            encoder(self.in_channels,N_s,3,1,1),
