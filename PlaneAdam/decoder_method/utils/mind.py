...
def MINDSSC(img, radius=2, dilation=2):
    ...
    six_neighbourhood = torch.Tensor([[0,1,1],
                                      ...
                                      [1,1,2],
                                      [2,1,1],
                                      [1,2,1]]).long().to(img.device)
    
    # squared distances
    dist = pdist_squared(six_neighbourhood.t().unsqueeze(0)).squeeze(0).to(img.device)
    
    # define comparison mask
    x, y = torch.meshgrid(torch.arange(6), torch.arange(6))
    x = x.to(img.device)
    y = y.to(img.device)
    mask = ((x > y).view(-1) & (dist == 2).view(-1)).to(img.device)

    # build kernel
    idx_shift1 = six_neighbourhood.unsqueeze(1).repeat(1,6,1).view(-1,3)[mask,:]
    idx_shift2 = six_neighbourhood.unsqueeze(0).repeat(6,1,1).view(-1,3)[mask,:]
    mshift1 = torch.zeros(12, 1, 3, 3, 3).to(img.device)
    mshift1.view(-1)[torch.arange(12).to(img.device) * 27 + idx_shift1[:,0] * 9 + idx_shift1[:, 1] * 3 + idx_shift1[:, 2]] = 1
    mshift2 = torch.zeros(12, 1, 3, 3, 3).to(img.device)
    mshift2.view(-1)[torch.arange(12).to(img.device) * 27 + idx_shift2[:,0] * 9 + idx_shift2[:, 1] * 3 + idx_shift2[:, 2]] = 1
    ...
    mind = mind[:, torch.Tensor([6, 8, 1, 11, 2, 10, 0, 7, 9, 4, 5, 3]).long().to(img.device), :, :, :]
