# EXPERIMENTAL VARIANT of convex_run_paired_dino.py, adding an image-space NCC term to the Adam
# refinement stage. Every other ConvexAdam script (this one's base included) optimizes purely in
# DINO feature space - correlate()/coupled_convex() and the Adam stage's feature-SSD term both
# only ever compare fixed vs. moving *features*, never the raw images. That means precision is
# capped by whatever the (patch_size=16-tokenized, then further grid_sp-pooled) feature grid can
# represent. The raw image was never patchified at all, so it doesn't suffer that quantization -
# this variant adds `lambda_ncc * NCC(fixed_image, warped_moving_image)`, computed at *native*
# image resolution (not the coarse grid_sp_adam-pooled grid the feature term uses), as an extra
# term in the Adam loss. Reuses decoder_method's NccLoss rather than reimplementing local NCC.
#
# Only wired into the Adam stage (--adam), since it's a differentiable gradient loop already -
# the discrete stage (correlate/coupled_convex) is a combinatorial candidate search over
# feature-space cost volumes, and adding an image term there would mean building a second,
# separate cost volume; a much bigger change, not attempted here. --lambda-ncc without --adam
# has nothing to attach the term to, so main() rejects that combination.
import argparse
import sys
import time
import warnings
    ...
    with torch.enable_grad():
        for _ in range(n_iters):
            ...
            if use_ncc:
                ...
                # uses) and warp the raw moving image with it - differentiably, so the NCC term's
...
def main(gpunum, configfile, pca_dim=None, adam_niter=None, lambda_ncc=0.0, ncc_win=9):
    ...
    run_name = f'dice{best_dice:.3f}{param_suffix}{pca_suffix}{adam_suffix}{ncc_suffix}_{time.strftime("%Y%m%d_%H%M%S")}'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--adam", type=int, default=None,
                         help="Run N iterations of ConvexAdam's Adam instance-optimization "
                              "refinement stage after the discrete stage. Required for --lambda-ncc "
                              "to have any effect. Omit to skip (discrete-only, current default behavior).")
    parser.add_argument("--lambda-ncc", type=float, default=0.0,
                         help="Weight of an image-space local-NCC term added to the Adam stage's loss, "
                              ...
                              "reproducing plain convex_run_paired_dino.py's Adam stage. Requires --adam.")
    ...
    main(args.gpu, args.configfile, args.pca, args.adam, args.lambda_ncc, args.ncc_win)
