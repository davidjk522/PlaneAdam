# EXPERIMENTAL VARIANT of convex_run_paired_dino.py, testing a second hypothesis (see the cosine
# variant for the first, which came back negative): PlaneCycle's cross-plane self-attention mixing
# (cycling HW/DW/DH across blocks to build global 3D context) may be diluting the local positional
# specificity that correlation-based registration needs — DINO-Reg, which works well, does NOT do
# anything like this; it encodes each 2D slice completely independently. This file swaps feature
# extraction from extract_feature_planecycle (PlaneCycle) to a plain per-slice 2D forward pass with
# no cross-slice/cross-plane attention at all, and changes nothing else — same discrete stage, same
# Adam stage (still raw SSD, matching the baseline, not the cosine variant), same PCA, same
# everything else — so any difference in results isolates the effect of PlaneCycle's mixing
# specifically. See convex_run_paired_dino.py for the baseline.
import argparse
import sys
import time
import warnings
...
DINO_TARGET_LONG_EDGE = 320  # in-plane (H,W) resize budget before DINO feature extraction - see the
...
def extract_feature_slice2d(backbone, volume, device):
    """
    ...
    with torch.no_grad():
        ...
        patch_tokens = backbone.forward_features(volume.to(device))['x_norm_patchtokens']  # (B*D, H_tok*W_tok, C)
        C = patch_tokens.shape[-1]
        H_tok = H // backbone.patch_size
        W_tok = W // backbone.patch_size
        return patch_tokens.view(B, D, H_tok, W_tok, C)
...
def main(gpunum, configfile, pca_dim=None, adam_niter=None):
        ...
        try:
            for i in trange(len(pairs)):
                ...
                with torch.no_grad():
                    ...
                    # Previously resized to a fixed 320x320 square, which forced two different stretch
                    # factors onto our non-square native H,W (160,224) - H ~2x vs W ~1.4x. Fixed by
                    # applying one uniform scale factor (pinned to the longer axis, W, so the 320
    ...
    run_name = f'dice{best_dice:.3f}{param_suffix}{pca_suffix}{adam_suffix}_slice2d_{time.strftime("%Y%m%d_%H%M%S")}'
