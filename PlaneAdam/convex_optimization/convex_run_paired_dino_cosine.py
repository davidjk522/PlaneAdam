# EXPERIMENTAL VARIANT of convex_run_paired_dino.py, testing one specific hypothesis: the Adam
# refinement stage's raw-SSD (squared L2) similarity loss, inherited unchanged from code tuned for
# MIND's bounded/normalized descriptor, may not fit DINO-style feature statistics well (feature
# vector *magnitude* can vary spatially for reasons unrelated to anatomical identity, which L2
# picks up on but shouldn't). This file swaps that one loss term for cosine similarity (scale-
# invariant, only compares feature *direction*) and changes nothing else — same discrete stage,
# same feature extraction/PCA, same everything else — so any difference in results isolates the
# effect of the similarity metric specifically. See convex_run_paired_dino.py for the baseline.
import argparse
import sys
import time
import warnings
...
def adam_refine_displacement(disp_init, features_fix_native, features_mov_native, H, W, D,
    ...
    displacement parameter, diffusion regularization, Adam), EXCEPT the similarity loss: the
    original (and our baseline convex_run_paired_dino.py) uses raw SSD; this variant uses cosine
    similarity between the fixed and warped-moving feature vectors instead, since SSD is sensitive
    to feature vector *magnitude* (which can vary spatially for reasons unrelated to anatomical
    identity for a learned ViT embedding) while cosine similarity only compares *direction*.

    Unlike the discrete stage (correlate/coupled_convex), this runs with gradients enabled and
    directly minimizes the warped-vs-fixed feature dissimilarity, so it can represent any
    real-valued displacement rather than only the discrete correlate() candidate set.
    """
    ...
    with torch.no_grad():
        ...
        patch_features_fix = F.normalize(patch_features_fix, dim=1, eps=1e-8)
    ...
    with torch.enable_grad():
        for _ in range(n_iters):
            ...
            # cosine similarity in [-1, 1] per voxel; 1 - cos_sim is 0 when perfectly aligned
            # (direction matches exactly) and grows as the feature directions diverge, regardless
            # of either vector's magnitude.
            patch_mov_sampled_norm = F.normalize(patch_mov_sampled, dim=1, eps=1e-8)
            cos_sim = (patch_mov_sampled_norm * patch_features_fix).sum(dim=1)
            sampled_cost = 1.0 - cos_sim
...
def main(gpunum, configfile, pca_dim=None, adam_niter=None):
        ...
        try:
            for i in trange(len(pairs)):
                ...
                with torch.no_grad():
                    ...
                    # 16), the same lever MedDINOv3 validated (640x640 -> 896x896, +2.06% DSC on AMOS22)
                    # to give the ViT a finer native token grid, directly targeting the coarse-quantization
                    # bottleneck diagnosed earlier. D is left untouched (it was never patchified to begin
                    # with). MedDINOv3's literal 896x896 and even 512x512 both OOM'd on this 12GB GPU
                    # during extraction itself (before correlate() is ever reached) — 320x320 is what
                    # actually fits here, still a real jump from the native 10x14 token grid to 20x20.
                    # Note: our native H,W (160,224) isn't square like MedDINOv3's CT slices, so this
                    # resize stretches H ~2x vs W ~1.4x — a real aspect-ratio distortion, flagged rather
                    # than silently applied.
                    fixed_volume = F.interpolate(fixed_volume, size=(D, 320, 320), mode='trilinear', align_corners=False)
                    moving_volume = F.interpolate(moving_volume, size=(D, 320, 320), mode='trilinear', align_corners=False)
    ...
    run_name = f'dice{best_dice:.3f}{param_suffix}{pca_suffix}{adam_suffix}_cosine_{time.strftime("%Y%m%d_%H%M%S")}'
