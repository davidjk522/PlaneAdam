# EXPERIMENTAL VARIANT of convex_run_paired_dino_slice2d.py, testing higher input resolution on top
# of the Slice2D (no-PlaneCycle) extraction that already proved to be the session's biggest win.
# Native token grid goes from 20x20 (320x320 input, patch_size=16) to 40x40 (640x640 input) — the
# same 640x640 MedDINOv3 used as its *lower* baseline resolution (before their own 896x896 step).
#
# 640x640 needs chunked extraction to fit in 12GB: the unchunked version pushes all D=192 slices
# through the ViT as one (B*D, tokens, C) batch, and attention memory scales with slice count x
# tokens-per-slice^2. At 320x320 (400 tokens/slice) that fit; at 640x640 (1600 tokens/slice, 4x)
# it's a large enough jump that it's not worth risking without headroom, so extract_feature_slice2d
# below processes D in chunks instead of all at once — peak memory becomes bounded by chunk_size,
# not by the full D=192, at the cost of more forward-pass calls (chunking trades memory for time,
# it doesn't reduce total compute). See convex_run_paired_dino_slice2d.py for the 320x320 version
# this was copied from, and convex_run_paired_dino.py for the original PlaneCycle baseline.
import argparse
import sys
import time
import warnings
...
def extract_feature_slice2d(backbone, volume, device, chunk_size=SLICE_CHUNK_SIZE):
    ...
    of one (B*D, tokens, C) batch — at 640x640 (1600 tokens/slice, 4x the 320x320 case) pushing all
    ...
    (~470MB at 40x40 tokens x 192 x 384 channels x fp32) — chunking only bounds the *transient*
...
def main(gpunum, configfile, pca_dim=None, adam_niter=None):
        ...
        try:
            for i in trange(len(pairs)):
                ...
                with torch.no_grad():
                    ...
                    # giving a 40x40 native token grid, double the 20x20 the 320x320 version used. D is
    ...
    run_name = f'dice{best_dice:.3f}{param_suffix}{pca_suffix}{adam_suffix}_slice2d640_{time.strftime("%Y%m%d_%H%M%S")}'
