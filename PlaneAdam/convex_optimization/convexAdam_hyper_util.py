# util function of convexAdam_hyper_util.py, copied directly from the source code of convexAdam_hyper_util.py, with minor modifications to remove unused imports and functions.
import time
import warnings
...
def cupy_hd95(fixed, moving, num_labels, precision=1):
    import cupy  # only needed here; kept out of module-level imports so scripts that never call this can run without it
...
def inverse_consistency(disp_field1s,disp_field2s,iter=20):
    ...
    with torch.no_grad():
        ...
        # the sampling grid of each voxel's own coordinates -> starting point with no displacement
        ...
        for i in range(iter):
            ...
            # identity + disp_field1s = for every voxel x, the location x + u(x) samples 
            disp_field1i = 0.5*(disp_field1s - F.grid_sample(disp_field2s,(identity+disp_field1s).permute(0,2,3,4,1)))
            # identity + disp_field2s = for every voxel x, the location x + u(x) samples
            disp_field2i = 0.5*(disp_field2s - F.grid_sample(disp_field1s,(identity+disp_field2s).permute(0,2,3,4,1)))
#F.grid_sample: a lookup-by-coordinate function that samples input at user-specificed coodinates
# when the index is not an integer, interpolates
