...
def main(gpunum, configfile, pca_dim=None, adam_niter=None,
        ...
        try:
            for i in trange(len(pairs)):
                ...
                with torch.no_grad():
                    ...
                    # giving a 56x56 native token grid, matching the PlaneCycle-chunked variant's
                    # resolution (see convex_run_paired_dino_planecycle_chunked.py). D is left
                    # untouched (it was never patchified to begin with). Combined with chunked
                    # extraction (see extract_feature_slice2d/SLICE_CHUNK_SIZE above) — was 512x512 to
                    # fit a 12GB GPU, pushed to this 896 step now that we're on an A5000 (24GB); watch
                    # memory here.
    ...
    run_name = f'dice{best_dice:.3f}{param_suffix}{pca_suffix}{adam_suffix}{ncc_suffix}{mind_suffix}_slice2d896_{time.strftime("%Y%m%d_%H%M%S")}'
